...
- **시스템 트레이 연동**: 앱 닫기 시 즉시 종료되지 않도록 `close_action` 설정을 도입(`ask`, `minimize`, `exit`). 창 닫기 시 선택 다이얼로그를 통해 '트레이로 최소화' 및 '프로그램 종료'를 수행할 수 있게 조치함.
- **환경 설정 UI**: '파일 - 설정' 메뉴에 '환경 설정' 탭을 추가하고, 여기에 '프로그램 종료 시 시스템 트레이로 최소화' 옵션을 추가했습니다.
- **스플래시 및 로딩 UI 개선**: 스플래시 창이 무조건 최상단에 뜨도록 속성(`WindowStaysOnTopHint`) 적용.

## [2026-03-02] v0.1.1 애플리케이션 신규 빌드 및 파이프라인 테스트
- 버전 정보를 v0.1.0에서 v0.1.1로 업데이트 (pyproject.toml, ui/splash_screen.py, scripts/installer.iss)
- 단위 테스트 실행 및 결과 검증 (`test_tags.py` 내의 Tuple 반환형 변경 건 수정 반영, 10개 테스트 모두 통과)
- `scripts/build_app.ps1` 스크립트를 통한 빌드 자동화 및 통합 패키징 수행