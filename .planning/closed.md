...
- [x] 모니터링 중인 폴더 안에 파일 다운로드 테스트 한 결과 원본파일이 큐에 등록되었다가 바로 삭제되는 현상 발견됨. 
...
# 임베딩 어댑터 구현 및 임베딩 로직 구체화
- [x] 이미지 임베딩 로직 구현 필요.
    - 이미지 임베딩은 적당히 랜덤 돌리는 mock 구현으로 되어있는데, 실제 임베딩 모델을 사용하여 구현 필요.
    - qwen_adapter는 지금 멀티모달이 지원되는 임베딩 모델을 전제로 하고 있으므로 해당 임베딩 모델을 그대로 사용하면 될 것으로 보이나 직접 이미지 지원 여부를 다시 확인 후 적용 바람.

- [x] 텍스트 임베딩 로직 리뷰
    - 전체적으로 임베딩 로직이 어떻게 적용되어있는지 정리해서 공유 바람.
    - PDF는 페이지 단위로, 워드는 문단 단위로 (paragraph) 임베딩을 생성하도록 되어있는데, 이 방식이 적절한지 검토 필요.(페이지나 문단이 너무 길 경우에 대한 처리도 필요할 것으로 보임)
    - 그외 텍스트 파일로 분류되는 파일들 (.txt, .md, .py, .c, .cpp, .h, .java, .js, .html, .css)은 파일 전체를 하나의 텍스트로 간주하여 임베딩을 생성하도록 되어있는데, 이 방식이 적절한지 검토 필요.
        - 파일이 너무 길 경우에 대한 처리도 필요할 것으로 보임
        - 소스코드 파일들의 경우 임베딩 모델이 소스코드 원문을 그대로 임베딩 했을 때 유의미한 임베딩이 생성되는지도 의문. 관련 내용 확인 필요.
